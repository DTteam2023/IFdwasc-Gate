...
export class AccountStateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.secondFormGroup = this.fb.group({
      AccountStatementReceipt: ['', Validators.required],
    });
  ...
  onKeyReceipt(event: any) { // without type info
    ...
    if (this.firstFormGroup.get('AccountId').value != '') {
      ...
      this.appService.GetAccStat(event.target.value).subscribe((data) => {
        if (data.length == 0) {
  ...
  submit() {
    ...
    this.userData.AccountStatementReceipt = (document.getElementById('AccountStatementReceiptImg') as HTMLImageElement).src
    
    if (this.userData) {
      this.loader.open('اضافة شهادة كشف حساب للعميل');
      this.appService.PostAccStatData(this.userData)
        .subscribe(data => {
          this.loader.close();
