...
export class JwtAuthService {
  ...
  public signin(username, password) {
    return this.http.post(`${environment.apiURL}/api/auth/Login`, { username, password })
      .pipe(
        map((res: any) => {
          this.setUserAndToken(res.token, this.getUserData(res.token), !!res);
  ...
  */
  public checkTokenIsValid() {
    return of(this.DEMO_USER)
      .pipe(
        map((profile: User) => {
          this.setUserAndToken(this.getJwtToken(), this.getUserData(this.getJwtToken()), true);
          this.signingIn = false;
          return profile;
        }),
        catchError((error) => {
  ...
  getUserData(token: string) {
    ...
    this.DEMO_USER.id = this.payload.Id;
    this.DEMO_USER.displayName = this.payload.UserName;
    this.DEMO_USER.role = this.payload.Roles;
